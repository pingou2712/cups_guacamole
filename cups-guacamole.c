/* cups-guacamole.c -- CUPS Backend (version 3.0.1, 2017-02-24)
   08.02.2003, Volker C. Behr
   volker@cups-guacamole.de
   http://www.cups-guacamole.de
...
//strncpy(Conf_Out, "/home/pingou/thinclient_drives/GUACFS/Download/", BUFSIZE);
...
static int init(char *argv[]) {
  ...
  if ((uri != NULL) && (strncmp(uri, "cups-guacamole:/", 16) == 0) && strlen(uri) > 16) {
    uri = uri + 16;
    sprintf(filename, "%s/cups-guacamole-%s.conf", CP_CONFIG_PATH, uri);
  } 
  else {
    sprintf(filename, "%s/cups-guacamole.conf", CP_CONFIG_PATH);
  }
  ...
  if (strlen(Conf_Log)) {
    ...
    snprintf(filename, BUFSIZE, "%s/%s%s%s", Conf_Log, "cups-guacamole-", getenv("PRINTER"), "_log");
...
static void announce_printers() {
  ...
  printf("file cups-guacamole:/ \"Virtual PDF Printer\" \"CUPS-GUACAMOLE\" \"MFG:Generic;MDL:CUPS-GUACAMOLE Printer;DES:Generic CUPS-GUACAMOLE Printer;CLS:PRINTER;CMD:POSTSCRIPT;\"\n");

  if ((dir = opendir(CP_CONFIG_PATH)) != NULL) {
    while ((config_ent = readdir(dir)) != NULL) {
      len = strlen(config_ent->d_name);
      if ((strncmp(config_ent->d_name, "cups-guacamole-", 15) == 0) &&
          (len > 20 && strcmp(config_ent->d_name + len - 5, ".conf") == 0)) {
        strncpy(setup, config_ent->d_name + 15, BUFSIZE>len-20 ? len-20 : BUFSIZE);
        setup[BUFSIZE>len-20 ? len-20 : BUFSIZE - 1] = '\0'; 
        printf("file cups-guacamole:/%s \"Virtual %s Printer\" \"CUPS-GUACAMOLE\" \"MFG:Generic;MDL:CUPS-GUACAMOLE Printer;DES:Generic CUPS-GUACAMOLE Printer;CLS:PRINTER;CMD:POSTSCRIPT;\"\n", setup, setup);
      }
    }
    closedir(dir);
  }
...
int main(int argc, char *argv[]) {
  ...
  if (setuid(0)) {
    (void) fputs("CUPS-GUACAMOLE cannot be called without root privileges!\n", stderr);
    return 0;
  }
  ...
  if (argc<6 || argc>7) {
    (void) fputs("Usage: cups-guacamole job-id user title copies options [file]\n", stderr);
    return 0;
  }
  ...
  if (user == NULL) {
    (void) fputs("CUPS-GUACAMOLE: failed to allocate memory\n", stderr);
    return 5;
  }  
